...
#include "cSMHdCKMRHN_slha_io.hpp"
#include "cSMHdCKMRHN_model_slha.hpp"

#ifdef ENABLE_TWO_SCALE_SOLVER
#include "cSMHdCKMRHN_two_scale_model.hpp"
#include "cSMHdCKMRHN_two_scale_spectrum_generator.hpp"
#endif
...
namespace cSMHdCKMRHN_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FScSMHdCKMRHNMessage", 1);
...
class cSMHdCKMRHN_spectrum {
public:
   virtual ~cSMHdCKMRHN_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(cSMHdCKMRHN_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const cSMHdCKMRHN_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const cSMHdCKMRHN_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class cSMHdCKMRHN_spectrum_impl : public cSMHdCKMRHN_spectrum
{
public:
   virtual ~cSMHdCKMRHN_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(cSMHdCKMRHN_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const cSMHdCKMRHN_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const cSMHdCKMRHN_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<cSMHdCKMRHN_slha<cSMHdCKMRHN<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   cSMHdCKMRHN_scales scales{};              ///< scale information
   cSMHdCKMRHN_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const cSMHdCKMRHN_input_parameters& input_) { input = input_; }
...
private:
   cSMHdCKMRHN_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<cSMHdCKMRHN_spectrum> spectrum{nullptr};  ///< spectrum information

   cSMHdCKMRHN_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FScSMHdCKMRHNCalculateSpectrum[]?");
   }
}
...
void Model_data::put_input_parameters(MLINK link) const
{
   ...
   MLPutRuleTo(link, INPUTPARAMETER(MvInput), "MvInput");
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, cSMHdCKMRHN_info::particle_names[i].c_str());
      }
   }
}
...
cSMHdCKMRHN_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   cSMHdCKMRHN_slha_io slha_io;
...
void put_spectrum(const cSMHdCKMRHN_slha<Model>& model, MLINK link)
{
   MLPutRule(link, cSMHdCKMRHN_info::model_name);
   MLPutFunction(link, "List", 50);
...
template <typename Solver_type>
void cSMHdCKMRHN_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void cSMHdCKMRHN_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const cSMHdCKMRHN_input_parameters& input
)
{
   cSMHdCKMRHN_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void cSMHdCKMRHN_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void cSMHdCKMRHN_spectrum_impl<Solver_type>::fill_slha_io(cSMHdCKMRHN_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   cSMHdCKMRHN_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FScSMHdCKMRHNCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FScSMHdCKMRHNCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new cSMHdCKMRHN_spectrum_impl<Two_scale>());
...
Model_data make_data(const Dynamic_array_view<Element_t>& pars)
{
   ...
   INPUTPARAMETER(MvInput(0,0)) = pars[c++];
   INPUTPARAMETER(MvInput(0,1)) = pars[c++];
   INPUTPARAMETER(MvInput(0,2)) = pars[c++];
   INPUTPARAMETER(MvInput(1,0)) = pars[c++];
   INPUTPARAMETER(MvInput(1,1)) = pars[c++];
   INPUTPARAMETER(MvInput(1,2)) = pars[c++];
   INPUTPARAMETER(MvInput(2,0)) = pars[c++];
   INPUTPARAMETER(MvInput(2,1)) = pars[c++];
   INPUTPARAMETER(MvInput(2,2)) = pars[c++];
...
DLLEXPORT int FScSMHdCKMRHNGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMRHNGetSettings"))
...
DLLEXPORT int FScSMHdCKMRHNGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMRHNGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMRHNGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMRHNGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMRHNOpenHandle(
...
DLLEXPORT int FScSMHdCKMRHNCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::cSMHdCKMRHN_librarylink;
...
DLLEXPORT int FScSMHdCKMRHNSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::cSMHdCKMRHN_librarylink;
...
DLLEXPORT int FScSMHdCKMRHNGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMRHNGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMRHNToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMRHNToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMRHNGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMRHNGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMRHNCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMRHNCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMRHNCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKMRHN_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMRHNCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FScSMHdCKMRHNCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FScSMHdCKMRHNCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FScSMHdCKMRHNCalculateObservables", "error", e.what());
      put_error_output(link);
   }

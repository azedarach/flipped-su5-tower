...
#include "cSMHdCKMRHN_two_scale_low_scale_constraint.hpp"
...
cSMHdCKMRHN_low_scale_constraint<Two_scale>::cSMHdCKMRHN_low_scale_constraint(
...
void cSMHdCKMRHN_low_scale_constraint<Two_scale>::apply()
{
   check_model_ptr();
...
const Eigen::Matrix<std::complex<double>,3,3>& cSMHdCKMRHN_low_scale_constraint<Two_scale>::get_ckm()
{
   return ckm;
}

const Eigen::Matrix<std::complex<double>,3,3>& cSMHdCKMRHN_low_scale_constraint<Two_scale>::get_pmns()
{
   return pmns;
}

double cSMHdCKMRHN_low_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double cSMHdCKMRHN_low_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void cSMHdCKMRHN_low_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<cSMHdCKMRHN<Two_scale>*>(model_);
}

void cSMHdCKMRHN_low_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& cSMHdCKMRHN_low_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void cSMHdCKMRHN_low_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
...
void cSMHdCKMRHN_low_scale_constraint<Two_scale>::initialize()
{
   check_model_ptr();
...
void cSMHdCKMRHN_low_scale_constraint<Two_scale>::update_scale()
{
   check_model_ptr();
...
void cSMHdCKMRHN_low_scale_constraint<Two_scale>::calculate_threshold_corrections()
{
   check_model_ptr();
...
double cSMHdCKMRHN_low_scale_constraint<Two_scale>::calculate_theta_w()
{
   check_model_ptr();
...
void cSMHdCKMRHN_low_scale_constraint<Two_scale>::calculate_DRbar_gauge_couplings()
{
   check_model_ptr();
   ...
   if (IsFinite(new_g1)) {
      ...
      model->get_problems().flag_non_perturbative_parameter(
         cSMHdCKMRHN_info::g1, new_g1, get_scale());
   ...
   if (IsFinite(new_g2)) {
      ...
      model->get_problems().flag_non_perturbative_parameter(
         cSMHdCKMRHN_info::g2, new_g2, get_scale());
      new_g2 = Electroweak_constants::g2;
   }
}

double cSMHdCKMRHN_low_scale_constraint<Two_scale>::calculate_delta_alpha_em(double alphaEm) const
{
   check_model_ptr();
...
double cSMHdCKMRHN_low_scale_constraint<Two_scale>::calculate_delta_alpha_s(double alphaS) const
{
   check_model_ptr();
...
double cSMHdCKMRHN_low_scale_constraint<Two_scale>::calculate_alpha_s_SM5_at(
...
void cSMHdCKMRHN_low_scale_constraint<Two_scale>::calculate_running_SM_masses()
{
   check_model_ptr();
...
void cSMHdCKMRHN_low_scale_constraint<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void cSMHdCKMRHN_low_scale_constraint<Two_scale>::calculate_Yu_DRbar()
{
   check_model_ptr();
...
void cSMHdCKMRHN_low_scale_constraint<Two_scale>::calculate_Yd_DRbar()
{
   check_model_ptr();
...
void cSMHdCKMRHN_low_scale_constraint<Two_scale>::calculate_Ye_DRbar()
{
   check_model_ptr();

   const auto v = MODELPARAMETER(v);
   MODEL->set_Ye(((1.4142135623730951*downLeptonsDRbar)/v).template cast<std::
      complex<double> >());

}

void cSMHdCKMRHN_low_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("cSMHdCKMRHN_low_scale_constraint<Two_scale>: "

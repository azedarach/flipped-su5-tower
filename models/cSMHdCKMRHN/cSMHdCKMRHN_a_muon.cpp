...
#include "cxx_qft/cSMHdCKMRHN_qft.hpp"
...
using namespace cSMHdCKMRHN_cxx_diagrams;
...
double get_MSUSY(const cSMHdCKMRHN_mass_eigenstates& model)
{
   return 0.;
}

void run_to_MSUSY(cSMHdCKMRHN_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("cSMHdCKMRHN_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("cSMHdCKMRHN_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("cSMHdCKMRHN_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("cSMHdCKMRHN_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(cSMHdCKMRHN_mass_eigenstates& model)
{
   VERBOSE_MSG("cSMHdCKMRHN_a_muon: calculating a_mu at Q = " << model.get_scale());

   context_base context{ model };
   double val = 0.0;
   
   using namespace cSMHdCKMRHN_cxx_diagrams::fields;
...
std::pair<double,double> vary_scale(const cSMHdCKMRHN_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("cSMHdCKMRHN_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double cSMHdCKMRHN_a_muon::calculate_a_muon(const cSMHdCKMRHN_mass_eigenstates& model_)
{
   cSMHdCKMRHN_mass_eigenstates model(model_);

   VERBOSE_MSG("cSMHdCKMRHN_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("cSMHdCKMRHN_a_muon:" << e.what());
...
double cSMHdCKMRHN_a_muon::calculate_a_muon_uncertainty(const cSMHdCKMRHN_mass_eigenstates& model_)
{
   cSMHdCKMRHN_mass_eigenstates model(model_);

   VERBOSE_MSG("cSMHdCKMRHN_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("cSMHdCKMRHN_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }

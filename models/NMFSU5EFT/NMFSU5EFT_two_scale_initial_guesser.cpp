#include "NMFSU5EFT_two_scale_initial_guesser.hpp"
#include "NMFSU5_two_scale_model.hpp"
#include "NMFSU5EFT_two_scale_matching.hpp"
...
NMFSU5EFT_initial_guesser<Two_scale>::NMFSU5EFT_initial_guesser(
   NMFSU5<Two_scale>* model_,
   cSMHdCKMRHN<Two_scale>* intermediate_model_,
   cSMHdCKM<Two_scale>* eft_,
   const softsusy::QedQcd& qedqcd_,
   const NMFSU5EFT_low_scale_constraint<Two_scale>& low_constraint_,
   const NMFSU5EFT_susy_scale_constraint<Two_scale>& susy_constraint_,
   const NMFSU5EFT_high_scale_constraint<Two_scale>& high_constraint_
)
   : Initial_guesser()
   , model(model_)
   , intermediate_model(intermediate_model_),
   , eft(eft_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("NMFSU5EFT_initial_guesser: Error: pointer to model"
                       " NMFSU5<Two_scale> must not be zero");
   if (!intermediate_model)
      throw SetupError("NMFSU5EFT_initial_guesser: Error: pointer to model"
                       " cSMHdCKMRHN<Two_scale> must not be zero");
   if (!eft)
      throw SetupError("NMFSU5EFT_initial_guesser: Error: pointer to model"
                       " cSMHdCKM<Two_scale> must not be zero");
}
 ...
 */
void NMFSU5EFT_initial_guesser<Two_scale>::guess()
{
 ...
 * \endcode
 */
void NMFSU5EFT_initial_guesser<Two_scale>::guess_eft_parameters()
{
...
void NMFSU5EFT_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void NMFSU5EFT_initial_guesser<Two_scale>::calculate_running_SM_masses()
{
 ...
 * corrections).
 */
void NMFSU5EFT_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
 ...
 */
void NMFSU5EFT_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
 ...
 * corrections).
 */
void NMFSU5EFT_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
 ...
 */
void NMFSU5EFT_initial_guesser<Two_scale>::guess_model_parameters()
{

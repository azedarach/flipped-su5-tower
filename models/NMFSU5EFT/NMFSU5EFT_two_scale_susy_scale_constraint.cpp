...
NMFSU5EFT_susy_scale_constraint<Two_scale>::NMFSU5EFT_susy_scale_constraint(
...
void NMFSU5EFT_susy_scale_constraint<Two_scale>::apply()
{
   check_model_ptr();
...
double NMFSU5EFT_susy_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double NMFSU5EFT_susy_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

const NMFSU5EFT_input_parameters& NMFSU5EFT_susy_scale_constraint<Two_scale>::get_input_parameters() const
{
   return input;
}

cSMHdCKMRHN<Two_scale>* NMFSU5EFT_susy_scale_constraint<Two_scale>::get_model() const
{
   return model;
}

void NMFSU5EFT_susy_scale_constraint<Two_scale>::set_model(Model* model_)
{
   model = cast_model<cSMHdCKMRHN<Two_scale>*>(model_);
}

void NMFSU5EFT_susy_scale_constraint<Two_scale>::set_sm_parameters(
   const softsusy::QedQcd& qedqcd_)
{
   qedqcd = qedqcd_;
}

const softsusy::QedQcd& NMFSU5EFT_susy_scale_constraint<Two_scale>::get_sm_parameters() const
{
   return qedqcd;
}

void NMFSU5EFT_susy_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
...
void NMFSU5EFT_susy_scale_constraint<Two_scale>::initialize()
{
   check_model_ptr();
...
void NMFSU5EFT_susy_scale_constraint<Two_scale>::update_scale()
{
   check_model_ptr();
...
double NMFSU5EFT_susy_scale_constraint<Two_scale>::calculate_initial_scale_guess() const
{
   check_model_ptr();
...
void NMFSU5EFT_susy_scale_constraint<Two_scale>::calculate_seesaw_Mv(
...
void NMFSU5EFT_susy_scale_constraint<Two_scale>::check_model_ptr() const
{
   if (!model)
      throw SetupError("NMFSU5EFT_susy_scale_constraint<Two_scale>: "

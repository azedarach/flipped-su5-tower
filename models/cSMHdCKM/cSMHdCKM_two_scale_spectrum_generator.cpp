...
#include "cSMHdCKM_two_scale_susy_scale_constraint.hpp"
...
double cSMHdCKM_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void cSMHdCKM_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const cSMHdCKM_input_parameters& input)
{
   ...
   cSMHdCKM_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<cSMHdCKM_ewsb_solver<Two_scale> >(ewsb_solver));

   cSMHdCKM_high_scale_constraint<Two_scale> high_scale_constraint(&model);
   cSMHdCKM_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   cSMHdCKM_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   high_scale_constraint.initialize();
   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   cSMHdCKM_convergence_tester<Two_scale> convergence_tester(
   ...
   cSMHdCKM_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void cSMHdCKM_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   cSMHdCKM_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_high_scale());
}

void cSMHdCKM_spectrum_generator<Two_scale>::calculate_spectrum()
{

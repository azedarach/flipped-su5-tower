...
cSMHdCKM_spectrum_plotter::cSMHdCKM_spectrum_plotter(const cSMHdCKM_mass_eigenstates& model)
{
   extract_spectrum(model);
}

void cSMHdCKM_spectrum_plotter::extract_spectrum(const cSMHdCKM_mass_eigenstates& model)
{
   spectrum.clear();
...
void cSMHdCKM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("cSMHdCKM_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("cSMHdCKM_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("cSMHdCKM_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void cSMHdCKM_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("cSMHdCKM_spectrum_plotter::write_spectrum: "
 ...
 */
void to_database(
   const std::string& file_name, const cSMHdCKM_mass_eigenstates& model,
   ...
   const cSMHdCKM_observables* observables)
{
   ...
   append(names, cSMHdCKM_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, cSMHdCKM_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill extra parameters
   append(names, cSMHdCKM_parameter_getter().get_extra_parameter_names());
   append(values, model.get_extra_parameters());

   // fill DR-bar masses and mixings
   append(names, cSMHdCKM_parameter_getter().get_DRbar_mass_names());
   append(names, cSMHdCKM_parameter_getter().get_DRbar_mixing_names());
   append(values, model.get_DRbar_masses_and_mixings());

   // fill pole masses and mixings
   append(names, cSMHdCKM_parameter_getter().get_pole_mass_names());
   append(names, cSMHdCKM_parameter_getter().get_pole_mixing_names());
   ...
   if (cSMHdCKM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, cSMHdCKM_observables::get_names());
 ...
 */
cSMHdCKM_mass_eigenstates from_database(
   const std::string& file_name, long long entry, softsusy::QedQcd* qedqcd,
   Physical_input* physical_input, cSMHdCKM_observables* observables)
{
   using utilities::append;

   cSMHdCKM_mass_eigenstates model;
   const auto number_of_parameters = model.get_number_of_parameters();
   const auto number_of_masses = cSMHdCKM_parameter_getter().get_number_of_masses();
   const auto number_of_mixings = cSMHdCKM_info::NUMBER_OF_MIXINGS;
   const auto number_of_input_parameters = cSMHdCKM_info::NUMBER_OF_INPUT_PARAMETERS;
   const auto number_of_extra_parameters = cSMHdCKM_info::NUMBER_OF_EXTRA_PARAMETERS;
   ...
   const auto number_of_observables =
      (observables ? cSMHdCKM_observables::NUMBER_OF_OBSERVABLES : 0);

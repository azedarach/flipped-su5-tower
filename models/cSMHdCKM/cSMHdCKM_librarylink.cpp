...
#include "cSMHdCKM_slha_io.hpp"
#include "cSMHdCKM_model_slha.hpp"

#ifdef ENABLE_TWO_SCALE_SOLVER
#include "cSMHdCKM_two_scale_model.hpp"
#include "cSMHdCKM_two_scale_spectrum_generator.hpp"
#endif
...
namespace cSMHdCKM_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FScSMHdCKMMessage", 1);
...
class cSMHdCKM_spectrum {
public:
   virtual ~cSMHdCKM_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(cSMHdCKM_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const cSMHdCKM_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const cSMHdCKM_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class cSMHdCKM_spectrum_impl : public cSMHdCKM_spectrum
{
public:
   virtual ~cSMHdCKM_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(cSMHdCKM_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const cSMHdCKM_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const cSMHdCKM_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<cSMHdCKM_slha<cSMHdCKM<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   cSMHdCKM_scales scales{};              ///< scale information
   cSMHdCKM_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const cSMHdCKM_input_parameters& input_) { input = input_; }
...
private:
   cSMHdCKM_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<cSMHdCKM_spectrum> spectrum{nullptr};  ///< spectrum information

   cSMHdCKM_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FScSMHdCKMCalculateSpectrum[]?");
   }
}
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, cSMHdCKM_info::particle_names[i].c_str());
      }
   }
}
...
cSMHdCKM_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   cSMHdCKM_slha_io slha_io;
...
void put_spectrum(const cSMHdCKM_slha<Model>& model, MLINK link)
{
   MLPutRule(link, cSMHdCKM_info::model_name);
   MLPutFunction(link, "List", 46);
...
template <typename Solver_type>
void cSMHdCKM_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void cSMHdCKM_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const cSMHdCKM_input_parameters& input
)
{
   cSMHdCKM_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void cSMHdCKM_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void cSMHdCKM_spectrum_impl<Solver_type>::fill_slha_io(cSMHdCKM_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   cSMHdCKM_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FScSMHdCKMCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FScSMHdCKMCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new cSMHdCKM_spectrum_impl<Two_scale>());
...
DLLEXPORT int FScSMHdCKMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKM_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMGetSettings"))
...
DLLEXPORT int FScSMHdCKMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKM_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKM_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMOpenHandle(
...
DLLEXPORT int FScSMHdCKMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::cSMHdCKM_librarylink;
...
DLLEXPORT int FScSMHdCKMGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKM_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKM_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKM_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKM_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FScSMHdCKMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::cSMHdCKM_librarylink;

   if (!check_number_of_args(link, 1, "FScSMHdCKMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FScSMHdCKMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FScSMHdCKMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FScSMHdCKMCalculateObservables", "error", e.what());
      put_error_output(link);
   }

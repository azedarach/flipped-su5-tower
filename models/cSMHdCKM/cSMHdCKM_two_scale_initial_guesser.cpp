...
#include "cSMHdCKM_two_scale_initial_guesser.hpp"
...
cSMHdCKM_initial_guesser<Two_scale>::cSMHdCKM_initial_guesser(
   cSMHdCKM<Two_scale>* model_,
   const softsusy::QedQcd& qedqcd_,
   const cSMHdCKM_low_scale_constraint<Two_scale>& low_constraint_,
   const cSMHdCKM_susy_scale_constraint<Two_scale>& susy_constraint_,
   const cSMHdCKM_high_scale_constraint<Two_scale>& high_constraint_
)
   : model(model_)
   , qedqcd(qedqcd_)
...
{
   if (!model)
      throw SetupError("cSMHdCKM_initial_guesser: Error: pointer to model"
 ...
 */
void cSMHdCKM_initial_guesser<Two_scale>::guess()
{
 ...
 * \endcode
 */
void cSMHdCKM_initial_guesser<Two_scale>::guess_susy_parameters()
{
...
void cSMHdCKM_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_running_SM_masses();
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
...
void cSMHdCKM_initial_guesser<Two_scale>::calculate_running_SM_masses()
{
 ...
 * corrections).
 */
void cSMHdCKM_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
   const auto v = MODELPARAMETER(v);
   MODEL->set_Yu((((1.4142135623730951*upQuarksDRbar)/v).transpose()).template
 ...
 */
void cSMHdCKM_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
   const auto v = MODELPARAMETER(v);
   MODEL->set_Yd((((1.4142135623730951*downQuarksDRbar)/v).transpose()).template
      cast<std::complex<double> >());
 ...
 * corrections).
 */
void cSMHdCKM_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
   const auto v = MODELPARAMETER(v);
   MODEL->set_Ye((((1.4142135623730951*downLeptonsDRbar)/v).transpose()).template
      cast<std::complex<double> >());
 ...
 */
void cSMHdCKM_initial_guesser<Two_scale>::guess_soft_parameters()
{
